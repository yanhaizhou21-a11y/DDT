...
### 2.8 Project Tracker (multi-domain)
A general project-level tracker, separate from the global Dev Tracker in §2.1 — this module tracks **individual projects** (one card per project) rather than the whole GitHub account, and supports non-software work too.

- **Create a project** with: name, **domain type**, and a **status**.
- **Domain type** (picked at creation, changeable later): `Software Development`, `Graphic Design`, `Game Development`, `Video/Photo`. Domain type decides which activity metric the project tracks (below) — it is not just a label.
- **Status dropdown** — always exactly 3 states, label text adapts to domain type so it reads naturally for that kind of work:

  | Status (internal value) | Software | Graphic Design | Game Dev | Video/Photo |
  |---|---|---|---|---|
  | `not_started` | Just Started | Just Started | Just Started | Just Started |
  | `in_progress` | In Progress | In Progress | In Progress | In Progress |
  | `ready` | Ready to Deploy | Ready to Deliver | Ready to Ship | Ready to Deliver |

- **Per-project daily activity graph** — reuses the dot-ledger heatmap component from `design.md` §3, scoped to one project instead of the whole account:
  - `Software Development` / `Game Development` **with a linked GitHub repo:** metric = commits/day, pulled from the same GitHub integration in §2.1 (GraphQL `contributionsCollection` filtered to that repo, or per-repo commit REST endpoint), cached the same way (§6). No separate manual entry needed — this is the "commits per day" analytics the user asked for.
  - `Graphic Design` / `Video/Photo`, or a `Software`/`Game Dev` project with **no linked repo:** metric = manual daily activity count. A single "log today's work" input (e.g. "+3") lets the user record output (revisions, exports, clips edited, photos culled — the label is generic, "items logged today," not domain-prescriptive) without needing a third-party API.
- **Linking a repo:** optional field on `Software`/`Game Dev` projects, a dropdown of repos already visible from the connected GitHub token (§2.1). If no GitHub token is configured, this field is simply hidden and the project falls back to manual logging.
- **Project list view:** one card per project — name, domain icon, status badge (colored per §5 of `design.md`'s token set, not a new color), and the 30-day dot-ledger strip.
- **Project detail view:** full 12-month heatmap (same component as Dev Tracker), the status dropdown, and either the live commit count (repo-linked) or the manual log input (everything else).
- **Acceptance:** changing a project's domain type immediately swaps the status-dropdown labels and the activity-input mode (commit-pull vs. manual) with no reload; a repo-linked project never shows a manual-entry input (avoids double-counting the same work two ways).
...
| `projects` | id, name, domain_type (`software`/`graphic_design`/`game_dev`/`video_photo`), status (`not_started`/`in_progress`/`ready`), linked_repo (nullable text — repo full name), created_at |
| `project_activity` | id, project_id (fk), date (YYYY-MM-DD), count (int), source (`github`/`manual`) |
...
8. **Project Tracker** (§2.8) — build after GitHub integration (step 5) since repo-linked projects reuse it; the manual-logging path has no dependency and can be built/tested independently of GitHub being configured.
9. **Dashboard home** aggregation view (last, since it depends on every other module existing).
10. **v2 candidates (not built now, just noted):** Open Food Facts nutrition lookup, notifications/reminders, data export/import polish, dark/light theme toggle.
...
- All 7 modules function fully offline/without any API key configured, except the features that are inherently third-party-data-dependent (heatmap, poster art, cover art, repo-linked project commits) — those degrade to a "connect" prompt or fall back to manual entry, not a crash.
